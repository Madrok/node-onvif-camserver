...
export class Onvif {
	OnvifDevice = require('./modules/device.js');
	_OnvifSoap = require('./modules/soap.js');
	_MULTICAST_ADDRESS: string = '239.255.255.250';
	_PORT: number = 3702;
	_udp = null;
	_devices = {};
	_DISCOVERY_INTERVAL: number = 150; // ms
	_DISCOVERY_RETRY_MAX: number = 3;
	_DISCOVERY_WAIT: number = 3000; // ms
	_discovery_interval_timer: NodeJS.Timeout = null;
	_discovery_wait_timer: NodeJS.Timeout = null;
	_uuid: string = null;
	...
	private _isValidCallback(callback: any) {
		return (callback && typeof (callback) === 'function') ? true : false;
	};

	private _execCallback = function (callback: (v: any, v2: any) => void, arg1?: any, arg2?: any) {
		if (this._isValidCallback(callback)) {
			callback(arg1, arg2);
		}
	}

	private _sendProbe(callback?): Promise<void> {
		let soap_tmpl = '';
	...
	stopProbe(callback?: (v1: Error) => void): Promise<void> {
